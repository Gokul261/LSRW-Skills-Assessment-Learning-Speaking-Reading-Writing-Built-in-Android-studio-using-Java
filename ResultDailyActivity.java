...
public class ResultDailyActivity extends AppCompatActivity {
    ...
    private ArrayList<DailyTaskActivity.QuestionModel> questionList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        databaseRef = FirebaseDatabase.getInstance().getReference("daily_task_scores");

        // Receiving data from DailyTaskActivity
        int score = getIntent().getIntExtra("score", 0);
        questionList = (ArrayList<DailyTaskActivity.QuestionModel>) getIntent().getSerializableExtra("questions");
        ...
        finishButton.setOnClickListener(v -> {
            startActivity(new Intent(ResultDailyActivity.this, HomeActivity.class));
            finish();
        });
    }
    ...
    private void displayQuestionsWithAnswers() {
        for (int i = 0; i < questionList.size(); i++) {
            DailyTaskActivity.QuestionModel question = questionList.get(i);
    ...
    private void storeResultInFirebase(int score) {
        if (auth.getCurrentUser() != null) {
            ...
            resultData.put("daily_task_score", score);

            // Save data under the user's node
            userRef.setValue(resultData)
                    .addOnSuccessListener(aVoid ->
                            Toast.makeText(ResultDailyActivity.this, "Score saved successfully!", Toast.LENGTH_SHORT).show()
                    )
                    .addOnFailureListener(e ->
                            Toast.makeText(ResultDailyActivity.this, "Failed to save score: " + e.getMessage(), Toast.LENGTH_SHORT).show()
                    );
        } else {
